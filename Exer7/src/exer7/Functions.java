 ...
 public class Functions {
    ...
    public void GetResultSetSQL(String frameName, JTable jtableName){
        ...
        if (frameName.equalsIgnoreCase("studentsJFrame")){
            ...
            searchQuery = "SELECT Subjects.subjid, Subjects.subjode, Subjects.subjdesc, Subjects.subjunits, Subjects.subjsched, (SELECT COUNT(Subjects.subjid) from Enroll, Subjects WHERE Subjects.subjid=Enroll.subjid AND Enroll.subjid=Assign.subjid) as enrolled FROM Teachers, Subjects, Assign WHERE Assign.subjid=Subjects.subjid AND Teachers.Tid=Assign.Tid AND Teachers.Tid=" + LoginGUIJFrame.userId;
        }
    ...
    class FilterSQL {
       ...
       private List<String> teachersNameList=new ArrayList<>();
       private void SetTeachersNameFromResultSet(String teachersNameList){
           this.teachersNameList.add(teachersNameList);
       }
       
       public List<String> GetTeachersNameFromResultSet(){
           return teachersNameList;
       }
       
       public void ClearTeachersNameFromResultSet(){
           this.teachersNameList.clear();
       }
...
class DBPermissions {
    ...
    public void CreateTeacherUser(String Tid, String Tname){
        String autoGeneratedId = "t" + Tid + Tname;
    ...
    public void GrantTeacherPermission(String Tid, String Tname, String dbSelected){
        ...
        System.out.println(grantTeacherPermsSQL);
        db.executeUpdate(grantTeacherPermsSQL);
        // grants dummyDB perms in order to connect
        String grantDummyDBPermsSQL = "GRANT SELECT ON dummyDB.* TO '" + autoGeneratedId + "'@'localhost'"; 
        System.out.println(grantDummyDBPermsSQL);
        db.executeUpdate(grantDummyDBPermsSQL);
    }
